...
export function validateStoryData(chapters, scenes, endings, materials) {
  const _chapters = chapters || []
  const _scenes = scenes || {}
  const _endings = endings || []
  const _materials = materials || []
  const allWarnings = []
  allWarnings.push(..._checkBrokenChainScenes(_chapters, _scenes))
  allWarnings.push(..._checkMissingDialogues(_scenes))
  allWarnings.push(..._checkUnconfiguredAssets(_scenes, _materials, _chapters))
  allWarnings.push(..._checkUnreachableEndings(_endings, _chapters, _scenes, _materials))
  return allWarnings
}
...
export const useWarningStore = defineStore('warning', () => {
  ...
  function runFullValidation(chapters, scenes, endings, materials) {
    ...
    warnings.value = validateStoryData(chapters, scenes, endings, materials)
